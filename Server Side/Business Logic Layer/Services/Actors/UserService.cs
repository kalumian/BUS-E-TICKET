...
{
    public class UserService : GeneralService
    {
        protected readonly UserManager<AuthoUser> _userManager;
        protected readonly IHttpContextAccessor _httpContextAccessor;

        public UserService(UserManager<AuthoUser> userManager, IUnitOfWork unitOfWork, IHttpContextAccessor httpContextAccessor) : base(unitOfWork)
        {
        ...
        protected async Task<AuthoUser> RegisterAsync(RegisterAccountDTO registerAccountDTO, EnAccountStatus status = EnAccountStatus.Active)
        {
            ...
            Utilities.ValidationHelper.ValidatieEmail(registerAccountDTO.Email);
            Utilities.ValidationHelper.ValidatieNumber(registerAccountDTO.PhoneNumber);
        ...
        public string GetCurrentUserID()
        {
            ...
            // Retrieve the role claim
            var roleClaim = _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.NameIdentifier);
            if (roleClaim == null || string.IsNullOrEmpty(roleClaim.Value))
                throw new UnauthorizedAccessException("User role claim not found.");
        ...
        public async Task EnsureOwnershipAsync(string entityId)
        {
            // الحصول على معرف المستخدم المسجل الدخول
            string loggedInUserId = GetLoggedInUserId();

            // جلب الكيان بناءً على ID
            var entity = await _userManager.FindByIdAsync(entityId) ?? throw new NotFoundException($"account with ID {entityId} not found.");
